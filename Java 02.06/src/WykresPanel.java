...
public class WykresPanel extends JPanel {
    ...
    private int barGap = 10; // odstep s³upkow od siebie
    ...
    //funkcja tworzaca s³upek (konkretny obiekt) o zadanym podpisie, wysokosci i kolorze
    public void addHistogramColumn(String label, int value, Color color) {
        Bar bar = new Bar(label, value, color);
        bars.add(bar); // dodanie slupka do listy s³upków
    }
    ...
    public void layoutHistogram(int option) {
        ...
        for (Bar bar : bars) {
            ...
            Icon icon = new ColorIcon(bar.getColor(), barWidth, barHeight); // stworzenie obiektu s³upka (rysowanego)
            label.setIcon(icon);
            barPanel.add(label);
    ...
    // stworzenie typu Bar - obiektu posiadaj¹cego label, wartosc i kolor
    private class Bar {
    ...
    // odpowiada konkretnie za rysowanie prostok¹tów i cienia, czyli ca³ych s³upków
    private class ColorIcon implements Icon {
        ...
        // metoda odpowiadajaca za rysowanie wykresu - s³upków
